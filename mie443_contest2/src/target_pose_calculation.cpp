...
vector<float> target_pose(RobotPose robotPose, vector<float> target_object){
    ...
    float m_0_x = target_object.object_x + m*cos(phi); // x-coordinate of point m_0
    float m_0_y = target_object.object_y + m*sin(phi); // y-coordinate of point m_0
    ...
    float D = 0.5; // Specify search diameter around point m_0
    float m_i[n][2]; // Array contianing search points around point m_0
    ...
    ros::ServiceClient check_path = n.serviceClient<nav_msgs::GetPlan>("/move_base/NavfnROS/make_plan");
    nav_msgs::GetPlan srv;
    srv.request.start = start; // needs starting position as input
    srv.request.goal = goal; //needs destination
    check_path.call(srv);
    ...
    float phi = boxes.coords[min_index][3]; // Object's orientation
    if(phi >= 0 && phi < M_PI){
